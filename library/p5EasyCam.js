...
class EasyCam {
  ...
  update(){
    ...
    // 補間アクションはダンプアクションより優先順位が低いです
    // つまりリセット中にアクションを起こす場合そっちが先に実行されます

    // ていうかああなるほどわかった
    // これあれだ
    // インタラクションをリセット移動中にぶちこむと
    // リセットがキャンセルされて
    // 戻らないんだ
    // リセット途中の状態にインタラクションが適用されてアクションが破棄されるみたいです
    // リセット中はインタラクションさせないようにすべきだと思うけれど...
    // 個人的には
    // まずアクションを起こすとリセットの移動が始まって
    // リセット中はリセットを実行してそれ以外の処理はしない、ってやるべきかなと
    // 1秒くらいで終了させる感じで...
    // Damped使うならもっと別の...リンゼイさんのあれみたいなやつ
    // それ終わったら解放させる
    // 初期stateを記録しておいてそれとの差分を消していく感じ
    // 最初にorbitControlを実行した時の
    // しかし他のカメラ使う場合は？
    // カメラ切り替えの際に記録...そうするしかないんじゃない
    // フラグをリセットする
    // フラグだけでいいんじゃない
    // Dampedもその、減衰処理使えばduration要らないんじゃないかと思いますが
    // インタラクションでtrueにしてtrueの場合はリセット進行させて抜ける
    // 終わったらfalseにする、そうすれば実行されない
    // あとはtrueの場合に初期状態との距離を詰めるのをどう記述するか
    // だと思いますがこれも_orbitFreeみたく新しいメソッドを用意する形になるかなって思いますね
    // 以上です
    if(b_update){
      cam.timedRot .stop();
      cam.timedPan .stop();
