// 20221004
// いわゆる正四面体対称性。鏡映が基本ですね。
// 難しいです。
// まずpab,pbc,pcaで作られる基本領域というのは正四面体の一つの面の6分の1の領域で、
// 具体的には（図示すれば一目瞭然なんだけども）まずpabがz軸のちょっと上の方にあるんですね
// そんでそこからy軸に平行についーっと進むとpcaがあって、一方xz平面内をx軸側にちょっと下方気味にすーっといくとpbcがあるんです。
// んでこれら3つのなす、pabのとこが直角の直角三角形が基本領域の一つってやつです。
// ああもう、図示すればわかりやすいの！
// んでそれをnbやncで鏡映すると同じ面の6つの細分をすべて網羅するってわけさ。
// naを使うと隣の面に映る。こうしてすべての面が網羅される。鏡映ではそれをレイマーチングの距離関数に
// かませてやってるのさ。
// 問題はここから。これを使っていかにして様々な多面体を作るのか？？？

// それはまた、別の話...（こら）

// 逃げないで真面目に説明しなさい（分かりました）
// まあその、難しくないけどさ...
// まず重心ベクトルから説明しないと。重心ベクトルというのはもともとのpab,pbc,pcaこれらは違う名前の方がいい
// qab,qbc,qcaとでもつけようか。で、これらの補間結合（和が1）で点を取る。これ。で、これのna,nb,ncによる鏡映で得られる点を取るだろ。
// それらと結んで辺ができる。これを複製しまくってできる立体が要するに完成品ってわけ。
// そしてその時の面っていうのも複製でできる頂点からできるものになるのさ。というか重心ベクトルの複製が
// 頂点のすべて。
// たとえばこれによるとqab,qbc,qcaから出来るやつっていうのは、まずqabの場合辺の頂点すべてを結んでできるから正八面体。
// qbcの場合面の重心4つをすべて結ぶ。なので小さな正四面体（双対）。qcaの場合は元のそれ。そういうこと。
// たとえば各面を、辺を3等分して正六角形にしたかったら...楽だろ？然るべき点、何処取るべきか、わかるだろ？
// そういうことさ。以上！

// ...同じことをするのがBC3とH3ってわけなのさ。ただな、鏡映軸が無いと使えないんだよこれ。つまりな。
// 変形立方体ではこれはできないのさ。だって鏡映軸もってないもんあれ。...
// たとえば2次元でも回転だけで複製して平面図形作れるでしょう。あれの3次元版をやるだけなんです。基本は切頂です。
// 問題は回すのが難しいということだけ。

// だから回転だけですべての面を網羅する必要がある。それにはどうするかというと2次元で出来ることを3次元でやるだけ。
// 鏡映で距離取るときに鏡映でできる点をすべて網羅してそれらすべてに対して距離関数、てかsdfを適用してmin取るだろ？
// じゃねえや。基本領域に落とすんだった。落としたうえでそこで距離関数適用するんだったわ。マジ間違えてたわ。めんごめんご～
// ...
// だぁら同じようにするの。基本領域が...んー。鏡映で複製されないから楽だと思うんだけどな。回転で映りあうわけさ。
// 数は半分になる。だから基本領域は2倍の大きさになる。回転でそこに落とし込んだら後はやること一緒です。
// なぜなら鏡映を2回やると回転になるでしょう。退化ってやつです。

// 変形二十面体の場合も同じ。基本領域が2倍の大きさになって、あとは...同じ。難しいけどね。でもまあ、定数分かってますし、
// モデリングかなんかで作っちゃって、計算で出しちゃって、それ使えばいいんで、楽です...
...
// 複数の頂点など
// グローバルをP1, P2, P3, ...などとして事前にinitializeで決めちゃうと
// いいかも。もちろんUVWから決まるんだけど、いちいちレイマーチングの
// 際に計算するのは冗長でしょ、だから最初に決めちゃうの。
...
"  vec3 color = blue;" +
"  fold(p);" +
"  float t = dot(p - uniqueP, pab);" +
"  updateDist(color, t, skyblue, dot(p - uniqueP, pbc), 1);" +
"  updateDist(color, t, turquoise, dot(p - uniqueP, pca), 1);" +
"  updateDist(color, t, purple, halfBar(p - uniqueP, na, 0.03), 0);" +
"  updateDist(color, t, coral, halfBar(p - uniqueP, nb, 0.03), 0);" +
"  updateDist(color, t, khaki, halfBar(p - uniqueP, nc, 0.03), 0);" +
"  updateDist(color, t, navy, sphere(p - uniqueP, 0.05), 0);" +
"  return vec4(color, t);" +
"}" +
// 法線ベクトルの取得
...
"  transform(light);" +
// 初期化処理。
"  initialize(vec3(4.0, 1.0, 1.0) / 6.0, 2.0);" +
// マーチングの結果を取得。
