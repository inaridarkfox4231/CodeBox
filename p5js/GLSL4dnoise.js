...
// 4x2でやってみる
const DATASIZE_X = 128;
const DATASIZE_Y = 128; // 2べきである必要はないみたいです
// あとこれはデータ格納の練習で
...
"  float x = (mod(n, 128.0) + 0.5) / 128.0;" +
"  float y = (floor(n / 128.0) + 0.5) / 128.0;" +
...
"  float xi, yi, zi, wi, xf, yf, zf, wf;" +
"  float rxf, ryf, rzf; float of;" +
"  float result = 0.0;" +
"  float ampl = 0.5;" +
"  float n1, n2, n3, n4;" +
"  float ampFallOff = 0.5;" +
"  for(int i=0; i<4; i++){" +
"    xi=floor(x); yi=floor(y); zi=floor(z); wi=floor(w);" +
"    xf=fract(x); yf=fract(y); zf=fract(z); wf=fract(w);" +
"    of = xi + yi * 16.0 + zi * 256.0 + wi * 4096.0;" +
"    rxf = scaledCosine(xf);" +
"    ryf = scaledCosine(yf);" +
...
"    n3 = getPerlin(of);" +
"    n3 += rxf * (getPerlin(of + 1.0) - n3);" +
"    n4 = getPerlin(of + 16.0);" +
"    n4 += rxf * (getPerlin(of + 16.0 + 1.0) - n4);" +
"    n3 += ryf * (n4 - n3);" +

"    n2 += scaledCosine(zf) * (n3 - n2);" +

"    n1 += scaledCosine(wf) * (n2 - n1);" +
...
"  p *= 8.0;" +
"  float n = noise(p.x, p.y, uCount/60.0, p.x);" +
"  vec3 col = getRGB(0.65, floor(n*8.0)/8.0, 1.0);" +
