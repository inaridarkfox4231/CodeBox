...
const CREATE_FIGURE_TIME = 60;
const SLICE_COUNT = 24;
const SLICE_TIME = 16; // 半分で線を走らせその直後にスライスして
// 残りの半分で動かしつつ線を消す
const WAIT_FOR_DELETE_TIME = 12; // スライス終了から消去開始までの時間
const DELETE_TIME_MAX = 60; // 消去は左下から右上に向かって1つずつ
...
// width*0.5に特化させる
// rとthetaをぶち込んで直接boundaryとの境界を取得する
// _drawerに渡して線引いてもらう
function getIntersectionsWithBound(r, theta){
  const result = getIntersectionsWithSquare(width*0.5, r * cos(theta), r * sin(theta), r * cos(theta) - sin(theta), r * sin(theta) + cos(theta));
  let points = [];
  for(let i = 0; i < 4; i++){
    if(result[i] != undefined){ points.push(result[i]); }
  }
...
class Tessa{
  ...
  display(){
    ...
    if(this.active && this.delayCount == 0){
      _node.setMonoColor(this.bodyColor);
    }else{
    // 基本はこっち
      _node.setUVColor();
    }
