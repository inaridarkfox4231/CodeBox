...
function draw(){
  ...
  _tessellator.setRotation(-posPhi, 0);
...
function createConfigButton(rcp){
...
function createConfigButtons(){
  for(let i = 0; i < 20; i++){ // 8つ増えました。18と19はカメラ用。
    createConfigButton(buttonRecipe[i]);
  }
}
...
class Tessellator{
  ...
  createTess(mesh){
    ...
    g.x = gx * cos(t1) + gy * sin(t1);
    g.y = -gx * sin(t1) + gy * cos(t1);
    gy = g.y;
    g.y = gy * cos(t2) + gz * sin(t2);
    g.z = -gy * sin(t2) + gz * cos(t2);
    g.add(this.translate);
    //g.x = gx * cos(PI*0.25) + gy * sin(PI*0.25);
   // g.y = -gx * sin(PI*0.25) + gy * cos(PI * 0.25);
    this.tesses.push(new Tessa(tessName, g, this.rotationZ, this.rotationX, rotationFlag, moveFlag));
