...
class Tessellator{
  ...
  createTess(mesh){
    ...
    this.tesses.push(new Tessa(tessName, p, t1, t2, s, rotationFlag, moveFlag));
...
// 20220101. 帰ってからやります・・・・
class Tessa{
  constructor(name, position, rotZ = 0, rotX = 0, sizeScale = 1, rotationFlag = false, moveFlag = false){
    this.name = name;
    ...
    this.rotationZDiff = 0; // rotZの変化
    this.rotationXDiff = 0; // rotXの変化
  ...
  update(){
    ...
    // 回転～
    this.rotationZDiff += this.rotationSpeedZ;
  ...
  display(){
    ...
    _node.useTopology(this.name)
         .setAttribute()
         .setMatrix([{tr:[x, y, z]}, {rotZ:this.rotationZ}, {rotX:this.rotationX}, {scale:[this.sizeScale]}, {tr:[dx, dy, dz]}, {rotZ:this.rotationZDiff}, {rotX:this.rotationXDiff}])
         .setVertexColor()
         .bindIndexBuffer()
...
function mousePressed(){
  ...
  if(mouseButton == LEFT){
    ...
    if(cameraFlag){ return; } // カメラが機能してるときは何もしない
...
function mouseWheel(){
  if(cameraFlag){ posR = constrain(posR + event.delta*0.5, 320, 2560); }
}
